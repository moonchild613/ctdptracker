...
export default function App() {
  ...
  const [timer, setTimer] = useState({ running: false, seconds: 0, sessionStart: null });
  const [auxTimer, setAuxTimer] = useState({ active: false, seconds: 900 }); // 15min
  ...
  // 辅助链倒计时
  useEffect(() => {
    if (auxTimer.active && auxTimer.seconds > 0) {
      auxRef.current = setInterval(() => {
        setAuxTimer(t => {
          if (t.seconds <= 1) {
            clearInterval(auxRef.current);
            setModal("aux_fail");
    ...
    return () => clearInterval(auxRef.current);
  }, [auxTimer.active]);
  ...
  function activateSymbol() {
    if (auxTimer.active) {
      setAuxTimer({ active: false, seconds: 900 });
      clearInterval(auxRef.current);
    }
    setTimer({ running: true, seconds: 0, sessionStart: Date.now() });
  }

  function finishSession() {
    const mins = Math.round(timer.seconds / 60);
    setTimer({ running: false, seconds: 0, sessionStart: null });
  ...
  function triggerAux() {
    setAuxTimer({ active: true, seconds: 900 });
  }
  ...
  const timerDisplay = `${pad(Math.floor(timer.seconds/60))}:${pad(timer.seconds%60)}`;
  const auxMins = Math.floor(auxTimer.seconds / 60);
  const auxSecs = auxTimer.seconds % 60;
            ...
            {!auxTimer.active ? (
              ...
              <div style={styles.auxCountdown}>
                ...
                <div style={styles.auxProgressBar}>
                  <div style={{ ...styles.auxProgressFill, width: `${(auxTimer.seconds / 900) * 100}%` }} />
                </div>
